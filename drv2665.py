...
class Driver:
    ...
    def _validate_chipid(self):
        ...
        if chipid not in (5,7):
            raise RuntimeError("Failed to find DRV2665, check wiring!")

    def reset(self):
        self._write_u8(REGISTER_2, RESET)
        self._read_register1()
    ...
    def queue_empty_full(self) -> tuple[bool, bool]:
        register0_value = self._read_u8(REGISTER_0)
        return register0_value & _FIFO_EMPTY_MASK, register0_value & _FIFO_FULL_MASK

    def write_digital(self, value: int):
        self._write_s8(REGISTER_DATA, value)

    def _read_register1(self):
        self._register1_value = self._read_u8(REGISTER_1)
        
    def _write_register1(self):
        self._write_u8(REGISTER_1, self._register1_value)    
    
    def _read_register2(self):
        self._register2_value = self._read_u8(REGISTER_2)

    def _write_register2(self):
        self._write_u8(REGISTER_2, self._register2_value)        
    ...
    def _write_s8(self, address: int, val: int) -> None:        
        assert -127 < val < 128
        if val < 0:
            val += 0xFF # two's complement
        self._write_u8(address, val)